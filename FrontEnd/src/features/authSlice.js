...
export const forgotPassword = createAsyncThunk(
    ...
    async (forgotPassword, { rejectWithValue }) => {
        try {
            const {
                data: { message, resetPasswordCode, email },
            } = await api.post(`${VITE_LOCAL_SERVER_URL}/api/forgot-password`, forgotPassword);

            return { message, resetPasswordCode, email };
        } catch (error) {
            console.error("Error in forgotPassword request:", error); // Log the error for debugging
